...
class MappingTest implements RewriteTest {
    ...
    @Test
    void emptyObject() {
        rewriteRun(yaml("workflow_dispatch: {}"));
    }
    ...
    void scalarKeyAnchorInBrackets() {
        rewriteRun(yaml("foo: [start: start, &anchor buz: buz, *anchor: baz, end: end]"));
    }

    @Test
    void scalarEntryValueAnchorInBrackets() {
        rewriteRun(yaml("foo: [start: start, &anchor buz: buz, baz: *anchor, end: end]"));
    }
    ...
    void escapeSequences(String string) {
        rewriteRun(yaml("escaped-value: $string"));
    }
}
